        ...
        <div class="col-sm-12 col-md-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="mt-3" action="{{ route($type.'.profile.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf  
                        <div class="row">
                            <div class="col-sm-12 col-md-3">
                                <img src="{{ Auth::guard('admin')->user()->avatar ? asset('storage/'.Auth::guard('admin')->user()->avatar) : asset('assets/images/user.png') }}" alt="" 
                                  class="profile-image"
                                  id="avatar">
                                <div class="mb-3">
                            ...
                            <div class="col-sm-12 col-md-9">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control {{($errors->first('name') ? "border border-danger" : "")}}" 
                                        id="name" 
                                        value="{{ Auth::guard('admin')->user()->name ? Auth::guard('admin')->user()->name : '' }}" 
                                ...
                                <div class="form-group">
                                  ...
                                  <input type="email" class="form-control {{($errors->first('email') ? "border border-danger" : "")}}" 
                                    id="email" name="email"
                                    value="{{ Auth::guard('admin')->user()->email ? Auth::guard('admin')->user()->email : '' }}"
                    ...
                    <form class="mt-3" action="{{ route($type.'.password.update') }}" method="post">
                        @csrf
                        <div class="form-group">
                    ...
                    <form class="mt-3 d-flex justify-content-between align-items-end" method="POST" action="{{ route($type.'.change-algorithm') }}">
                        @csrf
                        <div class="form-group w-100">
                        <label for="select-algorithm">Select Algorithm</label>
                        <select class="form-select {{($errors->first('algorithm') ? "border border-danger" : "")}}" 
                            id="select-algorithm" name="algorithm">
                            <option value="">Select One</option>
                            <option value="1" {{ $settings->used_algorithm == 1 ? 'selected' : '' }}>SVM</option>
                            <option value="2" {{ $settings->used_algorithm == 2 ? 'selected' : '' }}>Naive Bayes</option>
                        </select>
                        @error('algorithm')
                            <small class="text-danger mb-2" role="alert">
                                <strong>{{ $message }}</strong>
